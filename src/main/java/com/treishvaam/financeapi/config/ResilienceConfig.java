...
/**
 * TARGET: Resilience4j Configuration
 * PURPOSE: Configures the "Circuit Breaker" pattern.
 * GOAL: If external APIs (FMP, NewsData) fail, these settings stop our server from hanging.
 */
@Configuration
public class ResilienceConfig {

    /**
     * Defines the default behavior for all circuit breakers in the system.
     * - Opens circuit (stops requests) if 50% of requests fail.
     * - Waits 30 seconds before testing if the external service is back up.
     * - Uses a sliding window of the last 10 requests to calculate the error rate.
     */
    @Bean
