...
public class MarketDataService {
    ...
    public void fallbackPythonUpdate(String triggerSource, Throwable t) {
        logger.error(
                "Circuit Breaker Open: Python script update skipped. Reason: {}", t.getMessage());
    ...
    public void fallbackMarketMovers(String market, String triggerSource, Throwable t) {
        ...
        apiFetchStatusRepository.save(
                new ApiFetchStatus(
                        "Market Movers", "SKIPPED", triggerSource, "Circuit Breaker Open"));
    }
    ...
    public Object fetchHistoricalData(String ticker) {
        ...
        try {
            Object freshData =
                    marketDataFactory.getHistoricalDataProvider().fetchHistoricalData(ticker);
            historicalDataCacheRepository.save(
                    new HistoricalDataCache(
                            ticker,
                            objectMapper.writeValueAsString(freshData),
                            LocalDateTime.now()));
            return freshData;
        } catch (Exception e) {
            apiFetchStatusRepository.save(
                    new ApiFetchStatus(
                            "Legacy Chart (" + ticker + ")", "FAILURE", "MANUAL", e.getMessage()));
    ...
    private boolean isPasswordValid(String rawPassword) {
        ...
        return passwordEncoder.matches(
                rawPassword,
                userRepository.findByUsername(userDetails.getUsername()).get().getPassword());
    }
